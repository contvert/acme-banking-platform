...
export function AccountsPanel({ admin }: { admin: Admin }) {
  ...
  const [draft, setDraft] = useState({ name: '', last4: '', balance: '0' });
      ...
      <Card style={{ marginBottom: 20 }}>
        <div className={`${s.grid} ${s.grid3}`}>
          <Field label="Nom du compte">
            <input className={s.input} value={draft.name} placeholder="Compte courant"
              onChange={(e) => setDraft({ ...draft, name: e.target.value })} />
          </Field>
          <Field label="4 derniers chiffres">
        ...
        <div className={p.headActions} style={{ marginTop: 16 }}>
          <button
            ...
            onClick={async () => {
              ...
              if (ok) setDraft({ name: '', last4: '', balance: '0' });
            }}
          >
            <Icon name="plus" size={13} /> Ajouter un compte
          </button>
        </div>
      </Card>

      <Card>
        {admin.config.accounts.length === 0 && <div className={s.empty}>Aucun compte.</div>}
          ...
          <div key={a.id} className={s.row}>
            <div className={s.rowMain}>
              ...
              <div className={s.rowMeta} style={{ marginTop: 6 }}>
                ••{a.last4} · {money(a.balance, a.currency)} · disponible {money(a.available, a.currency)}
                {a.pending ? ` · en attente ${money(a.pending, a.currency)}` : ''}
              </div>
            </div>
    ...
    <>
      <Card style={{ marginBottom: 20 }}>
        ...
        <div className={p.headActions} style={{ marginTop: 16 }}>
          ...
          {accounts.length === 0 && <span className={s.rowMeta}>Créez d’abord un compte.</span>}
        </div>
      </Card>

      <Card>
        {admin.config.cards.length === 0 && <div className={s.empty}>Aucune carte.</div>}
    ...
    <>
      <Card style={{ marginBottom: 20 }}>
        <div className={`${s.grid} ${s.grid3}`}>
          ...
          <Field label="Montant (négatif = sortie)">
        ...
        <div className={p.headActions} style={{ marginTop: 16 }}>
          <button className={`${p.btn} ${p.btnPrimary}`} type="button"
            ...
            <Icon name="plus" size={13} /> Ajouter une transaction
          </button>
          {accounts.length === 0 && <span className={s.rowMeta}>Créez d’abord un compte.</span>}
        </div>
      </Card>
      ...
      <Card>
        {admin.config.transactions.length === 0 && <div className={s.empty}>Aucune transaction.</div>}
...
export function NotificationsPanel({ admin }: { admin: Admin }) {
  ...
  return (
    <>
      ...
      <Card>
        {admin.config.notifications.length === 0 && <div className={s.empty}>Aucune notification.</div>}
