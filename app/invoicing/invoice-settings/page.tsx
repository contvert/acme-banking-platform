...
export default function InvoiceSettingsPage() {
  const tabs = useTabs([{ label: 'General' }, { label: 'Branding' }, { label: 'Defaults' }]);

  return (
    <Page title="Invoice settings" actions={[{ label: 'Save', icon: 'check', primary: true }]}>
      {tabs.node}
      ...
      {tabs.active === 'General' && (
        <>
          ...
          <Card style={{ maxWidth: 560, marginBottom: 24 }}>
            <label className={f.label} htmlFor="company">Company name</label>
            ...
            <Card style={{ padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                ...
                <span style={{ flex: '1 1 180px', minWidth: 0 }}>
                  <span style={{ display: 'block', fontSize: 16 }}>Accept card payments</span>
                  <span className={t.mutedWrap} style={{ fontSize: 13 }}>
                    Connect a processor to enable card payment per invoice.
                  </span>
                </span>
                <button className={p.btn} type="button">Set Up</button>
              </div>
            </Card>
            <Card style={{ padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                ...
                <span style={{ flex: '1 1 180px', minWidth: 0 }}>
                  <span style={{ display: 'block', fontSize: 16 }}>Invoicing API</span>
                  <span className={t.mutedWrap} style={{ fontSize: 13 }}>
                    Access the API and build automations.
                  </span>
                </span>
                <button className={p.btn} type="button">View docs</button>
              </div>
            </Card>
          </div>
        </>
      ...
      {tabs.active === 'Branding' && (
        <Card style={{ maxWidth: 560 }}>
          <label className={f.label} htmlFor="logo">Invoice logo</label>
          <input id="logo" className={f.select} type="file" />
          <label className={f.label} htmlFor="accent">Accent colour</label>
        ...
        <Card style={{ maxWidth: 700 }}>
          {TOGGLES.map((label, i) => (
            <div key={label} style={{
              ...
              <span className={`${t.status} ${t.statusOk}`}>On</span>
            </div>
          ))}
        </Card>
      )}
